...
export class CleanRingConnection {
  ...
  private async initialize(): Promise<void> {
    ...
    ringBleModule.setOnNotification((data: any) => {
      ...
      // Skip duplicates within 100ms
      if (dataKey === this.lastNotificationData &&
          (now - this.lastNotificationTime) < 100) {
        console.log('🔁 Duplicate notification filtered');
        return;
      }

      this.lastNotificationData = dataKey;
  ...
  private async sendCommand(command: string): Promise<void> {
    ...
    // Add 500ms delay before sending command to prevent overload
  ...
  public async disconnect(): Promise<void> {
    ...
    // Reset battery cache when device disconnects
    try {
      const { healthDataSyncService } = await import('../services/HealthDataSyncService');
      healthDataSyncService.resetBatteryCache();
    } catch (error) {
      console.error('Failed to reset battery cache:', error);
    }
  }
  ...
  private async handleDisconnection(previousState?: ConnectionState): Promise<void> {
    ...
    // Reset battery cache on any disconnection
    try {
      const { healthDataSyncService } = await import('../services/HealthDataSyncService');
      healthDataSyncService.resetBatteryCache();
    } catch (error) {
  ...
  public async fetchHistoricalData(): Promise<void> {
    ...
    // Check if we're already fetching to prevent multiple simultaneous requests
    const store = useRingStore.getState();
    if (store.isHistoricalDataLoading) {
    ...
    try {
      ...
      store.setIsHistoricalDataLoading(true);

      // Clear existing historical data before fetching new data
      // This prevents duplicate records from accumulating
      store.clearHistoricalData();
      ...
      if (count > 0) {
        console.log(`📥 Fetching ${count} historical records...`);
        // Then fetch data
        await this.sendCommand('historicalData');
        ...
        console.log('📭 No historical data available on device');
      }
    } finally {
      store.setIsHistoricalDataLoading(false);
    }
  }
