...
export const RingMainScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    } else if (!isConnected && hasAutoSwitched) {
      setHasAutoSwitched(false);
    }
  }, [isConnected]); // Remove hasAutoSwitched from deps to prevent loops
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if ((state === ConnectionState.CONNECTED || storeConnected) && !hasAutoSwitched) {
        // Don't log here as the store watcher above already logs the switch
        setActiveTab('health');
        setHasAutoSwitched(true);
      }
  ...
  // Separate effect for health sync to prevent duplicate triggers
  useEffect(() => {
    let syncStarted = false;

    const startHealthSync = async () => {
      if (syncStarted) {
        console.log('⚠️ Health sync already started, skipping duplicate trigger');
        return;
      }
      ...
      try {
        ...
        const syncStatus = healthDataSyncService.getSyncStatus();
        ...
        syncStarted = true;
        console.log('📊 Starting health data sync (single trigger)...');

        // Start sync after a delay to ensure device is ready
        ...
        console.log('✅ Health sync started with 30 min auto-sync interval');
      } catch (error) {
    ...
    // Only start sync when connected
    if (isConnected) {
      startHealthSync();
    }
